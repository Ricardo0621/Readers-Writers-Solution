...
int main() {
	...
	srand(time(NULL));
	sem_roomEmpty=sem_open(ROOMEMPTY, O_CREAT, 0600, 1);
	sem_mutex = sem_open(MUTEX, O_CREAT, 0600, 1);
	...
	for (i = 0; i <  MAXPERSONS; i++) {
		...
		if (readerorwriter(.5) == READER) { //Se modifica la variable de 
